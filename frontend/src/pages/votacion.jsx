...
// Datos estáticos de candidatos (temporal)
import { obtenerCandidatos } from "../services/candidatoService";
...
const Votacion = () => {
  ...
  const [errorCandidatos, setErrorCandidatos] = useState(null);
      ...
      if (initialized && keycloak?.token) {
        try {
          ...
          if (response.success) {
            ...
            setErrorCandidatos(
              response.error || "No se pudo cargar la lista de candidatos."
            );
          }
        } catch (err) {
          console.error("Error al cargar candidatos:", err);
          setErrorCandidatos(
            "No se pudieron cargar los candidatos. Intente más tarde."
          );
        } finally {
  ...
  const confirmarVoto = async () => {
    ...
    try {
      ...
      setModalOpen(false);
      setMensaje(
        `¡Voto registrado exitosamente! Has votado por: ${candidatoSeleccionado.partido}`
      );
  ...
  // Mostrar mensaje de carga de candidatos
  if (loadingCandidatos && !candidatos.length) {
    return (
      <div
        style={{
          display: "flex",
        ...
        {user?.haVotado ? (
          ...
          >
            ...
            <p style={{ fontSize: "1.2rem", color: "#555" }}>
              Gracias por ejercer tu derecho al voto. Tu participación ya ha
              sido registrada.{mensaje ? <br /> : ""}
              {mensaje && (
          ...
          <>
            ...
            <p style={{ color: "#000000", marginBottom: "40px" }}>
              Selecciona el candidato que mejor represente tus ideales para las
