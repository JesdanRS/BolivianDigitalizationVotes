...
import { obtenerCandidatos, crearCandidato, actualizarCandidato, eliminarCandidato } from '../services/candidatoService';
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                    <button onClick={onClose} style={{ padding: '8px 16px', borderRadius: '6px', border: '1px solid #e5e7eb', background: 'white', color: '#4b5563', cursor: 'pointer' }}>
                        Cancelar
                    </button>
                    <button onClick={onConfirm} style={{ padding: '8px 16px', borderRadius: '6px', border: 'none', background: '#dc2626', color: 'white', cursor: 'pointer' }}>
                        Confirmar
                    </button>
                </div>
            </div>
...
const GestionCandidatos = () => {
    ...
    const handleConfirmDelete = async () => {
        if (selectedCandidato) {
            try {
                const response = await eliminarCandidato(selectedCandidato.id, keycloak.token);
                if (response.success) {
                    cargarCandidatos();
    ...
    const handleSave = async (formData) => {
        try {
            ...
            if (selectedCandidato && selectedCandidato.id) {
                // Edición
                response = await actualizarCandidato(selectedCandidato.id, formData, keycloak.token);
            } else {
                // Creación
    ...
    if (!initialized) return <div style={{ padding: 40, textAlign: 'center' }}>Cargando Keycloak...</div>;

    if (!keycloak?.authenticated) {
        return (
            <div style={{ padding: 40, textAlign: 'center' }}>
                <h2>Acceso Denegado</h2>
                <button onClick={() => keycloak.login()} style={{ padding: '10px 20px', cursor: 'pointer' }}>Iniciar Sesión</button>
            </div>
        );
    }
                    ...
                    <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0 }}>
                        <thead>
                            <tr style={{ background: '#f8fafc' }}>
                                <th style={{ padding: '12px 24px', textAlign: 'left', fontSize: '0.875rem', color: '#4b5563', fontWeight: 600 }}>Presidente</th>
                                <th style={{ padding: '12px 24px', textAlign: 'left', fontSize: '0.875rem', color: '#4b5563', fontWeight: 600 }}>Vicepresidente</th>
                        ...
                        <tbody>
                            {candidatos.length === 0 && !loading && (
                                <tr><td colSpan="5" style={{ padding: 24, textAlign: 'center' }}>No hay candidatos</td></tr>
                            )}
                            {candidatos.map((candidato) => (
                                <tr key={candidato.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                                    <td style={{ padding: '16px 24px', fontSize: '0.875rem', color: '#111827' }}>
                                        <strong>{candidato.nombreCompletoPresidente}</strong><br />
                                        <small style={{ color: '#666' }}>{candidato.carnetPresidente}</small>
                                    </td>
                                    <td style={{ padding: '16px 24px', fontSize: '0.875rem', color: '#111827' }}>
                                        {candidato.nombreCompletoVicepresidente}
                                    </td>
                                    <td style={{ padding: '16px 24px' }}>
                                        <span style={{ color: '#dc2626', fontSize: '0.875rem', fontWeight: 'bold' }}>{candidato.partido}</span>
                                    </td>
